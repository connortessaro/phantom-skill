---
name: phantom
description: Give agents their own spending limits with Phantom (phantom.codes), an OpenAI-compatible inference API where the key is prepaid credit that can mint capped child keys for subagents and signs a receipt of which model served each call. Use when capping what a worker or subagent can spend, stopping a runaway loop with a hard dollar or per-minute ceiling, letting an agent fund its own inference with no card in the loop, or verifying which model actually answered.
when_to_use: Trigger on "give the subagent a budget", "cap what this agent can spend", "stop it burning through my credit", or per-task API keys that expire. Also on anything named `sk-phantom-`, child keys, `x-phantom-receipt`, an `insufficient_balance` 402 or `spend_rate_exceeded` 429 from phantom.codes, and the phantom-key CLI. Not for provider-side rate limits from OpenAI or Anthropic, billing your own SaaS customers, generic cloud cost alerts, or multi-agent orchestration with no money involved.
---
